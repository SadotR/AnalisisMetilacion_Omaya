...
It then performs a BLAST search using the extracted sequences.
=cut
...
# Define paths to the query file and the database
my $query_file = $scaffold_methylated;
my $db = 'nt';  # Nucleotide database

# Define the output file
my $output_file = 'blast_output.txt';

# Define the BLAST command
my $blast_command = "blastn -query $query_file -db $db -out $output_file -outfmt 6";

# Execute the BLAST command
system($blast_command) == 0 or die "Failed to execute BLAST: $!";

print "BLAST search completed. Results are saved in $output_file\n";
